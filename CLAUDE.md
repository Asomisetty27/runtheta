...
Other top-level dirs: `sim/` — standalone physics simulation of the E-LT lead-time testbed (lets the predictive-maintenance claim be validated *before* Sam's physical hardware exists, fall 2026; has its own README + tests). `deploy/` — production systemd install (`install.sh`, `theta-monitor.service`), Grafana/Prometheus provisioning, jobstats integration. `tools/` — one-off analysis scripts (B200 baseline, E009 validation). `supabase/` + `.supabase/` — opt-in telemetry backend (US-East).
...
- **Tests are characterization tests** that pin real incidents (e.g. `test_peer.py` reproduces the E009 blind-flag). If one breaks, the burden is to show the *new* behavior is more correct than the pinned incident — not to re-baseline it away.
