...
Physics basis (updated 2026-07-09 — the DGX B200 is AIR-cooled per the NVIDIA
DGX B200 User Guide: 10U, 1,550 CFM chassis fans, passive SXM6 sinks; the
earlier liquid-cold-plate narrative here described GB200 NVL, not this system):
  B200 SXM6 in DGX B200, 25°C inlet air:
    junction-to-air R_thermal(load) ≈ 0.060 C/W (10U passive sinks + fan wall)
    junction-to-air R_thermal(idle) ≈ 0.130 C/W (fan floor / natural convection)
    idle power (NV-hostengine, NVLink fabric active): ~88W
    T_j_idle = 25 + 0.130 * 88 = 36.4°C
    T_j_load = 25 + 0.060 * 900 = 79°C
    T_j_degraded = 25 + 0.074 * 900 = 91.4°C  (+23% thermal resistance)
    T_j_zombie   = 25 + 0.120 * 108 = 38.0°C   (CUDA context stuck, near-idle cooling)
...
INLET_AIR_C       = 25.0    # °C — DGX B200 inlet air (operating range 10-35°C)
COOLANT_INLET_C   = INLET_AIR_C  # legacy name kept for downstream references
R_JUNCTION_CW     = 0.060   # C/W — junction-to-inlet-air effective resistance under load
R_JUNCTION_IDLE_CW = 0.130  # C/W — idle (fan floor); air keeps the idle/load gap
...
def _b200_tj(power_w: float, r_mult: float = 1.0, r_base: float = R_JUNCTION_CW) -> float:
    """Compute B200 junction temperature from power + optional R_thermal multiplier."""
    return INLET_AIR_C + r_base * r_mult * power_w

T_J_IDLE      = _b200_tj(IDLE_POWER_W, r_base=R_JUNCTION_IDLE_CW)   # 36.4°C
T_J_LOAD      = _b200_tj(LOAD_POWER_W)                              # 79.0°C
T_J_DEGRADED  = _b200_tj(LOAD_POWER_W, DEGRADED_R_MULT)             # 91.4°C
T_J_ZOMBIE    = _b200_tj(ZOMBIE_POWER_W, r_base=0.120)              # 38.0°C
...
def make_sample(
    ...
    return RawSample(
        ...
        fan_speed_pct    = None,           # DGX B200: chassis fans, no per-GPU fan tach via NVML
        ecc_sbit         = 0,
        ecc_dbit         = 0,
        throttle_reasons = 0,
...
print(f"    T_j idle  = {T_J_IDLE:.1f} °C   (inlet {INLET_AIR_C}°C + R_idle·{IDLE_POWER_W}W)")
print(f"    T_j load  = {T_J_LOAD:.1f} °C   (inlet {INLET_AIR_C}°C + R_load·{LOAD_POWER_W}W)")
...
# Path C: BMC-as-T_ref (measured chassis inlet air via Redfish — best when creds exist)
...
if r_idle_bmc is not None and r_load_bmc is not None:
    ...
    print(f"\n  Path C — BMC T_ref={COOLANT_INLET_C}°C (chassis inlet air as reference):")
...
print(f"  DGX B200  ·  8 GPUs  ·  air-cooled 10U (1,550 CFM)  ·  25°C inlet")
print(f"{'═' * 64}")
