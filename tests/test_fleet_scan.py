...
Covers: the 3/3 detection on a synthetic HGX-like fleet (position structure +
...
# The CLI's module-level rich Console wraps at terminal width AND auto-
# highlights numbers, injecting ANSI codes mid-string ("≥<ansi>2</ansi> nodes"),
# so substring assertions broke depending on the invoking terminal. Pin a
# plain, wide, colorless console for deterministic test output.
import theta.cli as _cli
_cli.console = __import__("rich.console", fromlist=["Console"]).Console(
    width=300, no_color=True, highlight=False, force_terminal=False)
...
def _hgx_like_fleet():
    ...
    # looks fine; only position correction surfaces it. (The node-05:2 case.)
    for r in recs:
...
def test_fleet_scan_flags_position_masked_unit(tmp_path):
    ...
    p.write_text(json.dumps(_hgx_like_fleet()))
