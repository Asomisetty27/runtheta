...
def _fake_amdsmi(*, hotspot=80.0, vram=46.0, power=560.0, gfx=95.0, umc=40.0,
    ...
    class _TempType:  # enum-ish
        HOTSPOT = "hotspot"; JUNCTION = "junction"; EDGE = "edge"; VRAM = "vram"
    class _TempMetric:
        CURRENT = "current"
    class _ClkType:
        GFX = "gfx"; MEM = "mem"
    ...
    def _temp(h, sensor, metric):
        if sensor == "hotspot":
            return hotspot
        if sensor == "vram":
            return vram
        return 0.0
    m.amdsmi_get_temp_metric = guard("temp", _temp)
...
def test_vram_temp_maps_to_mem_temp_c(inject_amdsmi):
    """F22 instrument: HBM/VRAM temp lands in mem_temp_c (MI300X: the usable
    pair is (junction, vram) — there is no edge sensor on MI300-series)."""
    inject_amdsmi(_fake_amdsmi(hotspot=84.0, vram=47.0))
    samples, _, _ = _collect(None)
    assert samples[0].mem_temp_c == 47.0
    assert samples[0].temp_junction == 84.0


def test_vram_temp_absent_stays_none(inject_amdsmi):
    """A zero/absent VRAM sensor must yield None, never 0.0 (a fake reading
    would poison R_pkg downstream — same zero-default defense as the rest)."""
    inject_amdsmi(_fake_amdsmi(vram=0.0))
    samples, _, _ = _collect(None)
    assert samples[0].mem_temp_c is None
