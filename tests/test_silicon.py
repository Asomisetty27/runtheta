...
class TestMicroThrottleDetector:
    ...
    def test_throttle_cooldown(self):
        ...
        F21: samples carry hw_thermal_slowdown — bit-less mild suppression is
        normal DVFS and (correctly) no longer alerts."""
        detector = MicroThrottleDetector()

        # First alert at t=1000-1004
        for i in range(5):
            sample = self._sample(timestamp=1000 + i, clock_sm_mhz=1000, sm_clock_max_mhz=1500, throttle_reasons=0x40)
            alert = detector.update(sample)
        first_alert = alert
        ...
        for i in range(5, 10):
            sample = self._sample(timestamp=1100 + i, clock_sm_mhz=1000, sm_clock_max_mhz=1500, throttle_reasons=0x40)
            alert = detector.update(sample)
        assert alert is None
        ...
        sample = self._sample(timestamp=1310, clock_sm_mhz=1000, sm_clock_max_mhz=1500, throttle_reasons=0x40)
    ...
    def test_multiple_gpus_independent(self):
        ...
        for i in range(5):
            sample_g0 = self._sample(
                ...
                clock_sm_mhz=1000,
                sm_clock_max_mhz=1500,
                throttle_reasons=0x40,  # F21: evidence gate needs a culpable bit
            )
...
class TestMicroThrottleEvidenceGate:
    """F21 (2026-07-14): boost clocks are opportunistic — sustained heavy load
    legitimately runs 80-90% of max boost with NO throttle bits. An 8x A100
    campaign produced continuous false WARNINGs on all healthy GPUs until the
    reason-bit gate was added. These tests pin the gate."""

    def _sample(self, i, clock=1200, mx=1410, util=99.0, reasons=0):
        return RawSample(
            gpu_index=0, timestamp=1000.0 + i, temp_junction=70.0, power_w=400.0,
            util_pct=util, mem_util_pct=60.0, perf_state=0,
            clock_sm_mhz=clock, clock_mem_mhz=1512,
            throttle_reasons=reasons, sm_clock_max_mhz=mx,
        )

    def test_dvfs_below_boost_without_bits_never_alerts(self):
        """The A100 case: 85% of boost, 99% util, zero reason bits — healthy
        sustained load, must stay silent no matter how long it persists."""
        det = MicroThrottleDetector()
        for i in range(20):
            assert det.update(self._sample(i, clock=1200, mx=1410)) is None

    def test_thermal_bit_still_alerts(self):
        """Same suppression WITH hw_thermal_slowdown set → alert at sustained."""
        det = MicroThrottleDetector()
        alerts = [det.update(self._sample(i, reasons=0x40)) for i in range(6)]
        assert any(a is not None for a in alerts)
        fired = next(a for a in alerts if a)
        assert "hw_thermal_slowdown" in fired.message

    def test_deep_suppression_alerts_without_bits(self):
        """<60% of boost under load alerts even with an empty bitmask —
        defense on platforms whose throttle mask is unreliable."""
        det = MicroThrottleDetector()
        alerts = [det.update(self._sample(i, clock=800, mx=1410)) for i in range(6)]
        assert any(a is not None for a in alerts)

    def test_benign_bits_do_not_alert(self):
        """sync_boost / app_clock_setting are operator choices, not faults."""
        det = MicroThrottleDetector()
        for i in range(10):
            assert det.update(self._sample(i, reasons=0x10 | 0x02)) is None
