...
def select_collector(config, *, prefer: str | None = None, allow_demo: bool = True):
    ...
    Detection order (unless `prefer` overrides):
      ...
      2. AMD (amdsmi)   — real implementation in rocm_collector.py
    ...
    `allow_demo=False` makes the no-backend case a hard error instead of a
    silent fall-through to synthetic samples. Any caller whose output feeds a
    real decision (calibration, characterization, paid data capture) must set
    it — demo-mode data is indistinguishable from real data downstream.
    """
    ...
    if prefer == "demo":
        ...
        # An explicit backend choice is always allowed; `allow_demo` guards
        # only automatic fallback after hardware detection fails.
        coll._allow_demo = True  # type: ignore[attr-defined]
    ...
    if _rocm_available():
        return _rocm()
    # Fall back to demo mode — or refuse, for callers that need real silicon.
    if not allow_demo:
        raise RuntimeError(
            "No real GPU telemetry backend available (pynvml and amdsmi both "
            "unusable) and this operation refuses demo mode: synthetic samples "
            "would silently poison its output. Install/repair the vendor "
            "library (pynvml for NVIDIA, amdsmi matching the ROCm version for "
            "AMD) and retry."
        )
    return NVMLCollector(config)


def probe_gpu_name(index: int = 0) -> str | None:
    """Vendor-agnostic device-name probe: NVML first, then amdsmi.

    Returns None when no backend can name the device. Exists because device
    names gate hardware-profile resolution (hw_profiles.resolve_or_default):
    a pynvml-only lookup on an AMD host silently anchors the *default* profile
    even when a correct vendor profile (e.g. mi300x) is bundled.
    """
    if _nvml_available():
        try:
            import pynvml
        ...
        try:
            ...
            try:
                ...
                if index < len(handles):
                    info = amdsmi.amdsmi_get_gpu_asic_info(handles[index])
                    if isinstance(info, dict):
                        return info.get("market_name") or None
            finally:
                try:
                    amdsmi.amdsmi_shut_down()
                except Exception:
