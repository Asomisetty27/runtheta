...
_PROFILES: dict[str, ThermalProfile] = {
    ...
    # ── NVIDIA — B200 SXM6 (Blackwell, dual-die CoWoS-L) — AIR-COOLED DGX/HGX ──
    #
    # DEPLOYMENT REALITY (verified 2026-07-09, NVIDIA DGX B200 User Guide,
    # docs.nvidia.com/dgx/dgxb200-user-guide): the DGX B200 — the Cal Poly AI
    # Factory unit (4x DGX B200 BasePOD) — is an AIR-COOLED 10U system:
    # 1,550 CFM airflow, 6x 3.3 kW PSUs, operating range 10–35 °C, passively
    # sunk SXM6 modules cooled by chassis fans. The liquid cold-plate variant
    # of Blackwell is the GB200 NVL rack (see "gb200" profile below). The
    # earlier revision of this entry modeled B200 as liquid — that assumption
    # was wrong for every DGX/HGX B200 deployment.
    #
    # Physics basis (air, 25 °C inlet, 10U passive sinks + chassis fan wall):
    #   R_ja(load) ≈ 0.060 C/W → P_load 1000 W → T_j ≈ 25 + 60 = 85 °C
    #   R_ja(idle) ≈ 0.13 C/W  → P_idle   85 W → T_j ≈ 25 + 11 = 36 °C
    # Air cooling KEEPS the idle/load R_theta gap (fan curves + natural
    # convection at idle), unlike cold-plate parts — the classifier can use it.
    "b200": ThermalProfile(
        family="blackwell",
        canonical_name="B200-SXM6",
        vendor="nvidia",
        tdp_w=1000.0,  # DGX B200 configured TDP; HGX variants up to 1200W exist
        idle_floor_w=85.0,
        junction_max_c=100.0,
        expected_ambient_c=25.0,  # inlet AIR (DGX operating 10–35 °C; DC cold aisle ~22–27)
        rtheta_load_threshold=0.072,   # +20% over healthy 0.060 → degradation boundary
        rtheta_idle_threshold=0.24,    # zombie bound; healthy idle ~0.13 with fan floor
        rtheta_expected_under_load=0.060,  # physics-derived, pending E005+ measurement
        rtheta_expected_idle=0.13,
        rtheta_drift_warn_c_per_day=0.0003,
        rtheta_drift_crit_c_per_day=0.0012,
        cooling="air-passive",         # passive SXM sinks + chassis fan wall
        # idle_window works user-space with no BMC creds (the Cal Poly install
        # path); when Redfish inlet-air IS available, set_external_ambient()
        # upgrades T_ref to the measured chassis inlet — strictly better.
        t_ref_strategy="idle_window",
        drift_min_std=0.002,           # 2 mC/W floor (vs 10 mC/W for T4)
        confidence="extrapolated",
        notes=(
            "AIR-COOLED (DGX B200 User Guide: 10U, 1,550 CFM, 10–35 °C). Liquid Blackwell = gb200 profile.",
            "Dual-die CoWoS-L package — NVML may report per-die power (~450W each at full load).",
            "Idle/load R_theta gap EXISTS on air — unlike cold-plate parts.",
            "Prefer BMC/Redfish chassis inlet-air as T_ref when credentials exist; idle-window otherwise.",
            "Pending first-party Stage 2 measurement on Cal Poly DGX B200 (E005+).",
        ),
    ),

    # ── NVIDIA — GB200 NVL (Blackwell superchip, liquid cold-plate racks) ──
    #
    # Physics basis (liquid cold-plate, 20 °C coolant, T_ref = coolant inlet):
    #   R_thermal_junction_to_coolant ≈ 0.055 C/W (effective, both dies combined)
    #   P_idle ≈ 90 W   → T_j_idle ≈ 20 + 0.055 * 90   ≈ 25 °C
    #   P_load ≈ 1100 W → T_j_load ≈ 20 + 0.055 * 1100 ≈ 80 °C
    #   → R_theta(idle) ≈ R_theta(load): CONSTANT across load for liquid cooling —
    # the classifier cannot use the idle/load gap; classification relies on
    # P-state + power. Drift detection remains valid (TIM/cold-plate fouling
    # raises R_theta uniformly).
    #
    # T_ref must be the coolant inlet (BMC Redfish), NOT idle junction: with
    # T_ref = T_j_idle, idle ΔT ≈ 0 and R_theta is invalid (below MIN_DELTA_T).
    "gb200": ThermalProfile(
        family="blackwell",
        canonical_name="GB200-NVL",
        vendor="nvidia",
        tdp_w=1200.0,  # per-B200 in NVL72 racks (liquid-enabled envelope)
        idle_floor_w=90.0,
        junction_max_c=100.0,
        expected_ambient_c=20.0,  # facility coolant supply spec
        rtheta_load_threshold=0.066,   # +20% over healthy 0.055
        rtheta_idle_threshold=0.066,   # same boundary — no idle/load gap in liquid
        rtheta_expected_under_load=0.055,
        rtheta_expected_idle=0.055,
        rtheta_drift_warn_c_per_day=0.0003,
        rtheta_drift_crit_c_per_day=0.0012,
        cooling="liquid-cold-plate",
        t_ref_strategy="coolant_inlet",
        ...
        confidence="extrapolated",
        notes=(
            "Liquid cold-plate NVL rack part — R_theta constant across idle/load.",
            "T_ref must be coolant-inlet (BMC Redfish) not idle junction — see t_ref_strategy.",
            "Grace CPU shares the superchip power domain — verify NVML power scoping on GB200.",
        ),
    ),

    # ── AMD — MI250X (CDNA2, OAM, 2×GCD — the Frontier part) ──
    "mi250x": ThermalProfile(
        family="cdna2",
        canonical_name="MI250X-OAM",
        vendor="amd",
        tdp_w=560.0,           # AMD datasheet: 500 W typical / 560 W peak OAM
        idle_floor_w=90.0,
        junction_max_c=105.0,
        expected_ambient_c=20.0,
        rtheta_load_threshold=0.45,
        rtheta_idle_threshold=0.80,
        rtheta_expected_under_load=0.34,
        rtheta_expected_idle=0.65,
        rtheta_drift_warn_c_per_day=0.0007,
        rtheta_drift_crit_c_per_day=0.0035,
        cooling="liquid-cold-plate",  # Frontier/HPC deployments; air variants exist
        confidence="datasheet",
        notes=(
            "CRITICAL vendor quirk: amdsmi reports POWER at the package level "
            "(both GCDs aggregated) while temperature is per-die — R_theta must "
            "divide per-package. Source: ROCm amd-smi documentation / AMD blogs.",
            "Two GCDs per OAM package; peer comparison should treat the package "
            "as the unit, not the GCD.",
            "No public per-GPU MI250X telemetry DATASET exists for calibration "
            "(verified July 2026: the only public Frontier data is facility-level "
            "waste-heat, figshare 10.6084/m9.figshare.24391240). AMD's Device "
            "Metrics Exporter surfaces live per-GPU thermal-violation counters but "
            "not R_theta ground truth, so thresholds stay datasheet-derived until "
            "a fleet pilot provides real calibration.",
        ),
    ),

    # ── AMD — MI350X / MI355X (CDNA4, OAM, 2025) ──
    "mi350x": ThermalProfile(
        family="cdna4",
        canonical_name="MI350X-OAM",
        vendor="amd",
        tdp_w=1000.0,          # air-cooled OAM per AMD June 2025 launch
        idle_floor_w=100.0,
        junction_max_c=110.0,
        expected_ambient_c=22.0,
        rtheta_load_threshold=0.38,
        rtheta_idle_threshold=0.70,
        rtheta_expected_under_load=0.28,
        rtheta_expected_idle=0.58,
        rtheta_drift_warn_c_per_day=0.0007,
        rtheta_drift_crit_c_per_day=0.0035,
        cooling="air",
        confidence="datasheet",
        notes=(
            "CDNA4 launch specs (AMD, June 2025): MI350X 1000 W air-capable.",
            "amdsmi collector path identical to MI300X; junction temp direct.",
        ),
    ),
    "mi355x": ThermalProfile(
        family="cdna4",
        canonical_name="MI355X-OAM-DLC",
        vendor="amd",
        tdp_w=1400.0,          # direct-liquid-cooled per AMD June 2025 launch
        idle_floor_w=110.0,
        junction_max_c=110.0,
        expected_ambient_c=18.0,
        rtheta_load_threshold=0.30,
        rtheta_idle_threshold=0.60,
        rtheta_expected_under_load=0.22,
        rtheta_expected_idle=0.50,
        rtheta_drift_warn_c_per_day=0.0006,
        rtheta_drift_crit_c_per_day=0.0030,
        cooling="liquid-cold-plate",
        confidence="datasheet",
        notes=(
            "CDNA4 flagship (AMD, June 2025): 1400 W, DLC only.",
            "Liquid loop removes R_sa — expect compressed R_theta range like "
            "other DLC parts; peer-relative detection is the reliable signal.",
        ),
    ),
...
_MATCH_RULES: list[tuple[str, str]] = [
    ...
    ("h100",   "h100"),
    ("gb200",  "gb200"),   # MUST precede "b200": "gb200" contains "b200", so when
    #                        this rule was listed after it, GB200 (liquid) silently
    #                        matched the air-cooled b200 profile (dead rule).
    ...
    ("mi250",  "mi250x"),  # catches MI250X and MI250 (non-X, 500 W) — same CDNA2 profile
    ("mi350",  "mi350x"),
    ("mi355",  "mi355x"),
