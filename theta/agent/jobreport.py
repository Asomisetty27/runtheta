...
jobstats (an open-source Princeton University project adopted by many HPC sites)
...
Two input modes (CLI `theta report`):
  ...
  * export — read saved Prometheus query_range JSON (the format the E009
             validation study used) — offline, and the validation fixture for this code.

Method mirrors the validated E009 analysis pipeline:
...
def _node_id(instance: str) -> str:
    """Node id from a Prometheus instance label: drop the port, then any
    site-specific `<cluster>-` hostname prefix, so downstream grouping keys
    are cluster-agnostic (jobstats sites prefix node names differently)."""
    node = instance.split(":")[0]
    return re.sub(r"^[A-Za-z]+-", "", node)
        ...
        with Path(p).open(newline="") as fh:
            ...
            for row in reader:
                if jobid is not None and "jobid" in cols \
                        and str(row.get(cols["jobid"], "")) != str(jobid):
                    continue
                node = _node_id(str(row[cols["node"]])).strip()
...
def steady_rtheta(aligned: dict, ambient: float = DEFAULT_AMBIENT_C) -> list[GpuJobStat]:
    ...
    for (node, ordn), d in aligned.items():
        ...
        rs, ts, ps = [], [], []
        for T, P in zip(d["T"][warm_start:], powers, strict=True):
            if load_thresh < P and P > 0:
...
def build_report(jobid: str, stats: list[GpuJobStat],
    ...
    marginal degradation lives (on the real validation job, the third flag — a confirmed
