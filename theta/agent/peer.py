...
E009 (a production fleet of 64 matched-load H100s, operator de-identified)
closed both gaps with a
...
# catches an unambiguous outlier (E009's node-05:7, +58%) but MISSES units whose
# anomaly is masked by HGX baseboard-position structure: on the validation
# fleet, position
...
# node-05:2 was −2.5% within-node but +12% after position correction).
#
...
# E009 method; it reproduces all 3 validation-fleet flags at zero false positives.
...
def median_polish_z(
    ...
    oidx = {o: j for j, o in enumerate(ords)}

    # Sparse fleet → dense (node × ordinal) matrix; absent cells are NaN and
    # nanmedian ignores them. Every row/column has ≥1 cell by construction
    # (nodes/ords are derived from the cells), so no all-NaN slices occur.
    m = np.full((len(nodes), len(ords)), np.nan)
    for (node, ordn), (_gid, rt) in cells.items():
        m[nidx[node], oidx[ordn]] = rt
    mask  = ~np.isnan(m)
    grand = float(np.median(m[mask]))

    # Tukey two-way polish, vectorized: each sweep subtracts the row then the
    # column nanmedians in place — the matrix buffer is reused across all
    # iterations, so the loop allocates only the small per-sweep median vectors.
    # Measured vs the pure-Python original (identical outputs): 2.9× at 1024
    # GPUs, 5× at 4096; ~3× SLOWER at 72 GPUs where numpy setup dominates
    # (1.6ms → 5ms — irrelevant at fleet-service cadence, so no hybrid path).
    for _ in range(iterations):
        np.subtract(m, np.nanmedian(m, axis=1, keepdims=True), out=m)
        np.subtract(m, np.nanmedian(m, axis=0, keepdims=True), out=m)

    resid = m[mask]
    rmed  = float(np.median(resid))
    mad   = float(np.median(np.abs(resid - rmed)))
    sigma = max(MAD_K * mad, rel_floor * grand)

    return {
        cells[k][0]: round(float(m[nidx[k[0]], oidx[k[1]]]) / sigma, 2)
        for k in cells
    }
