...
class ROCmCollector:
    ...
    def _collect_one(self, amdsmi, idx: int, handle) -> RawSample:
        ...
        # HBM/VRAM temperature (F22 R_pkg numerator). On MI300-series there is
        # no EDGE sensor, so (junction, vram) is the usable pair; delta is
        # workload-dependent (see vault F22) — downstream must condition.
        mem_temp_c = None
        try:
            vram = amdsmi.AmdSmiTemperatureType.VRAM
            metric = amdsmi.AmdSmiTemperatureMetric.CURRENT
            mt = float(amdsmi.amdsmi_get_temp_metric(handle, vram, metric))
            if 0 < mt < 150:
                mem_temp_c = mt
        except Exception:
            pass
