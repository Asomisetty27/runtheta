...
E009 validation analysis surfaced. This module fills it the moment the workload spans
power, with no probe required.

The trick that makes it hardware-agnostic: don't fit the raw R_θ(P) curve (whose
shape is hardware-specific) — fit the **residual** from the empirically calibrated
healthy H100 curve (E009 validation fleet). A healthy unit has ~zero residual at every power. Then:
