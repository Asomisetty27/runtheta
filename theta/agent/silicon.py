...
MicroThrottleDetector
  ...
  AND the driver reports a culpable throttle reason (thermal, HW slowdown,
  power brake, power cap), it emits a WARNING with the reason decoded.

  Evidence gate (F21, 2026-07-14): boost clocks are opportunistic — a healthy
  A100 running sustained GEMM sits at 83-85% of max boost via normal DVFS with
  NO throttle bits set. Clock ratio alone therefore CANNOT distinguish
  degradation from ordinary sustained load; an 8x A100 campaign produced
  continuous false WARNINGs on every healthy GPU until the reason-bit gate was
  added. Deep suppression (< HARD_SUPPRESSION_THRESHOLD) still alerts without
  bits, as a defense on platforms whose bitmask is unreliable.
"""
...
from .metrics import AlertEvent, GPUState, RawSample
...
HARD_SUPPRESSION_THRESHOLD = 0.60  # below this, alert even without reason bits
# Throttle-reason bits that indicate a real constraint (vs opportunistic DVFS).
# sync_boost and app_clock_setting are operator/driver choices, not faults.
CULPABLE_THROTTLE_MASK = (
...
class MicroThrottleDetector:
    ...
    def update(self, sample: RawSample) -> Optional[AlertEvent]:
        ...
        # Evidence gate (F21): mild suppression under load is normal DVFS —
        # require a culpable reason bit, OR deep suppression as the fallback.
        culpable = bool(sample.throttle_reasons & CULPABLE_THROTTLE_MASK)
        suppressed = (
            sample.util_pct >= LOAD_THRESHOLD
            and (
                (efficiency < EFFICIENCY_THRESHOLD and culpable)
                or efficiency < HARD_SUPPRESSION_THRESHOLD
            )
        )
