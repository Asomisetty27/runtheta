        ...
        try:
            ...
            try:
                ...
                if "permission" in err_str or "access denied" in err_str or "no permission" in err_str:
                    raise _NVMLPermissionError(str(nvml_err)) from nvml_err
                elif "driver not loaded" in err_str or "not found" in err_str or "libcuda" in err_str:
                    raise _NVMLDriverError(str(nvml_err)) from nvml_err
                else:
                    raise
...
def step_first_reading(gpus: list[dict], baselines: dict) -> None:
    ...
    from theta.agent.collector import CollectorConfig, NVMLCollector
    from theta.agent.metrics import STATE_LABELS, enrich
...
def step_finish(sys_info: dict, gpus: list[dict], baselines: dict, alert_cfg: dict, opt_in_telemetry: bool = False, redfish_cfg: dict | None = None) -> None:
    redfish_cfg = redfish_cfg or {}
    ...
    # Launch commands. Demo telemetry is never persisted as an implicit
    # production setting, so it remains an explicit command-line choice.
    demo_mode = bool(sys_info.get("demo"))
    monitor_command = "theta monitor --demo" if demo_mode else "theta monitor"
    non_t4 = [g for g in gpus if "T4" not in g["name"]]
    calibrate_line = (
    ...
    ok("Theta is ready.")
    console.print(
        f"\n  [{DIM}]Run [bold]{monitor_command}[/] to start. Press [bold]Ctrl+C[/] to stop.[/]\n"
    )
    ...
    if want_launch:
        ...
        from theta.agent.daemon import AgentConfig, ThetaAgent
        cfg = AgentConfig(
